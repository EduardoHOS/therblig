...
The user subsequently requested a commit and PR. Remote inspection found that the repository was renamed to `EduardoHOS/therblig` and current `main` diverged from this local Studio implementation. The user explicitly chose to publish the preceding 22 local commits together with the home redesign. The broader PR was initially opened as a draft. Its two implementations have now been reconciled against main `7722884`; current integration results are recorded in [the reconciliation plan](2026-09-07-main-reconciliation.md). The results below describe the original home change before that integration.
