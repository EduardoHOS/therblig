...
await check('a cross-pool sequence flow is refused, and the bytes are identical', async () => {
  ...
  const ir = JSON.parse(JSON.stringify(await import('../../core/projection.mjs').then(async (m) =>
    m.project((await import('../../core/document.mjs').then((x) => x.parse(before.toString()))).definitions))));
...
await check('a refusal reports the rule, not just a rejection', async () => {
  const f = sandbox('miwg/C.2.0.bpmn');
  const { rev } = await readWithRev(f);
  const m = await import('../../core/document.mjs');
  const irm = await import('../../core/projection.mjs');
