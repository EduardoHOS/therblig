...
import { expectedFromOps } from '../../io/guard.mjs';
...
await check('it depends on no renderer, no DOM and no browser', async () => {
  const src = readFileSync(fileURLToPath(new URL('../../render/svg.mjs', import.meta.url)), 'utf8');
  ...
  // Precise on purpose: the core module is named document.mjs, so a bare
  // /document\.\w/ matches the import path and accuses the renderer of touching a DOM
  // it never mentions. The test tripped on a filename, not on a fact.
  assert(!/\bdocument\.(createElement|querySelector|getElementById|body)\b/.test(src),
    'render/svg.mjs touches the DOM');
  assert(!/\bwindow\.\w/.test(src), 'render/svg.mjs touches window');
