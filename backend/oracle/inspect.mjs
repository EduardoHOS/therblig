...
import { parse, walk, index } from '../core/document.mjs';
import { diCoverage } from '../core/placement.mjs';
...
// Whitespace is collapsed because BPMN names really do contain newlines — A.2.0 has a
// gateway named "Gateway\n(Split Flow)" — and a diagnostic that breaks across lines
// mid-sentence is unreadable in a terminal and unparseable in a log.
const name = (el) => (el?.name || el?.id || '(unnamed)').replace(/\s+/g, ' ').trim();
